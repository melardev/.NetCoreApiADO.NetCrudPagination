...
namespace ApiCoreAdoNetCrudPagination.Infrastructure.Services
{
    public class TodoService : ITodoService
    {
            ...
            TodoShow show = TodoShow.All)
        {
            ...
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                if (show == TodoShow.All)
                {
                    ...
                    command = new SqlCommand("Select Id, Title, Completed, CreatedAt, UpdatedAt From Todo ORDER BY CreatedAt " +
                                             $"OFFSET {offset} ROWS FETCH NEXT {pageSize} ROWS ONLY", connection);
                }
                else
                {
                    command = new SqlCommand("Select COUNT(*) From [dbo].[Todo] Where Completed=@Completed", connection);
        ...
        public async Task CreateTodoVulnerable(Todo todo)
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                string sql = $"Insert Into [dbo].[Todo] (Title, Description, Completed) Values " +
